...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -cc $BPF_CLANG -cflags $BPF_CFLAGS -target 386 -type event -type tcpevent -type latevent -type piddata bpf ringbuffer.c -- -I../headers
...
func main() {
	...
	// kp5, err := link.Kprobe(fn5, objs.TcpRcvStateProcess, nil)
	// if err != nil {
	// 	log.Fatalf("opening kprobe: %s", err)
	// }
	// defer kp5.Close()
	...
	rd3, err := ringbuf.NewReader(objs.bpfMaps.Piddatas)
	if err != nil {
		log.Fatalf("opening ringbuf reader: %s", err)
	}
	defer rd3.Close()

	rd4, err := ringbuf.NewReader(objs.bpfMaps.Latevents)
	if err != nil {
		log.Fatalf("opening ringbuf reader: %s", err)
	}
	defer rd4.Close()
	...
	go readLoopPidDatas(rd3)
	go readLoopLatEvents(rd4)
...
func readLoopPidDatas(rd *ringbuf.Reader) {
	// bpfEvent is generated by bpf2go.
	var event bpfPiddata
	for {
		record, err := rd.Read()
		if err != nil {
			if errors.Is(err, ringbuf.ErrClosed) {
		...
		// if event.Comm[0]==115 && event.Comm[1]==105 && event.Comm[2]==112 && event.Comm[3]==112 {
		log.Printf("ts: %d\ttgid: %d\tcomm: %s\n\n", event.Ts, event.Tgid, unix.ByteSliceToString(event.Comm[:]))
		// }
	}
}

func readLoopLatEvents(rd *ringbuf.Reader) {
	// bpfEvent is generated by bpf2go.
	var event bpfLatevent
	for {
		record, err := rd.Read()
		if err != nil {
			if errors.Is(err, ringbuf.ErrClosed) {
		...
		// if event.Comm[0]==115 && event.Comm[1]==105 && event.Comm[2]==112 && event.Comm[3]==112 {
		log.Printf("delta_us: %d\tcomm: %s\n\n", event.DeltaUs, unix.ByteSliceToString(event.Comm[:]))
		// }
	}
}
